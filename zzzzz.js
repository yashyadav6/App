...
// Exact API Matching + Smooth Red UI
// ==========================================================
...
// 🚨 EXACT VIP HEADERS FROM YOUR LOG
const NT_HEADERS = {
...
window.renderNextBatchChunk = async function() {
    ...
    for (let i of chunk) {
        ...
        try {
            // Exact matching for course-details
    ...
    try {
        ...
        async function scanFolderTree(fId, depth) {
            ...
            try {
                ...
                let data = await engineFetch(API.CONTENT, 'POST', exactPayload);
...
// ==========================================
// 4. VOD CONTENT FOLDERS (EXACT PAYLOAD)
// ==========================================
window.fetchFolderContent = async function(folderId) {
    ...
    const data = await engineFetch(API.CONTENT, 'POST', exactPayload);
    listDiv.innerHTML = "";
