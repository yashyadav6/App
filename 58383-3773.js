...
// Strict Video_Type: 3 Scanner + Exact Time-Lock
// ==========================================================
...
// ==========================================
// 3. LIVE FEED (DEEP FOLDER SCAN FOR VIDEO_TYPE 3 ONLY)
// ==========================================
async function fetchLiveFeed(courseId) {
    ...
    try {
        ...
        async function scanFolderTree(fId, depth) {
            ...
            try {
                ...
                items.forEach(item => {
                    ...
                    // STRICT CHECK: Is it Video Type 3?
                    let vType = parseInt(d.video_type || 0);

                    // Agar type 3 hai, toh hi Live Feed list me daalo
                    if (vType === 3) {
            ...
            } catch (e) { console.warn(`Folder skipped: ${fId}`); }
        }
        ...
        allLiveClasses.forEach(item => {
            ...
            // 🚨 ROOT TIME EXTRACTION (Jaise tumne bataya)
            let rawTime = item.live_from || d.live_from || 0;
            let liveFrom = parseInt(rawTime);
            if(isNaN(liveFrom)) liveFrom = 0;
            liveFrom = liveFrom * 1000; // Milliseconds

            let diff = liveFrom - now;
            let tagHtml = ''; let btnHtml = '';

            // STRICT TIME LOGIC FOR TYPE 3
            if (liveFrom > 0 && diff > 0) {
                let h = Math.floor(diff / 3600000);
                let m = Math.floor((diff % 3600000) / 60000);
                let tStr = h > 0 ? `${h}h ${m}m` : `${m}m`;
                
                tagHtml = `<span class="tag-scheduled" style="background:#f59e0b; padding:2px 6px; border-radius:4px; font-size:10px; color:#000; font-weight:bold; margin-left:5px;">🕒 STARTS IN ${tStr}</span>`;
                // BUTTON BLOCKED
                btnHtml = `<button class="btn-outline" disabled style="color:#aaa; border-color:#333; cursor:not-allowed; opacity: 0.6; background:transparent;"><i class="fas fa-lock"></i> Scheduled (${tStr})</button>`;
            } 
            else {
                // Time has come (Diff <= 0)
...
window.fetchFolderContent = async function(folderId) {
    ...
    items.forEach((item, idx) => {
        ...
        // RULE 1: STRICTLY VIDEO TYPE 3 (LIVE/SCHEDULED)
        if (vType === 3) {
            // TIME HAS NOT COME (BLOCKED)
            if (liveFrom > 0 && diff > 0) {
                let h = Math.floor(diff / 3600000);
                let m = Math.floor((diff % 3600000) / 60000);
                let tStr = h > 0 ? `${h}h ${m}m` : `${m}m`;
                
                liveTag = ` <span style="background:#f59e0b; padding:2px 6px; border-radius:4px; font-size:10px; color:#000; font-weight:bold; margin-left:5px;">🕒 STARTS IN ${tStr}</span>`;
                btnText = `<i class="fas fa-lock"></i> Scheduled (${tStr})`;
                routeToLivePHP = false; 
                isBtnDisabled = true;
            } 
            // TIME CROSSED -> LIVE NOW
            else {
        ...
        // RULE 2: ANY OTHER VIDEO IS RECORDED
        else if (isVideo) {
        ...
        let dateString = liveFrom > 0 ? new Date(liveFrom).toLocaleString() : (isVideo && vType === 3 ? "Time not set" : "");
        let dateHtml = dateString ? `<br><small style="color: var(--text-muted); font-size:11px;"><i class="far fa-calendar"></i> ${dateString}</small>` : "";
        ...
        if (isFolder) {
            ...
            actionHtml = `<button class="btn-outline" disabled style="color:#aaa; border-color:#333; cursor:not-allowed; opacity: 0.6;">${btnText}</button>`;
        } else {
