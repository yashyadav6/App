...
// Advance Sequential Deep Scanner + Parent ID Extractor
// ==========================================================
...
// ==========================================
// 3. LIVE FEED (ADVANCED SEQUENTIAL DEEP SCANNER)
// ==========================================
async function fetchLiveFeed(courseId) {
    ...
    liveContainer.innerHTML = '<div class="text-center py-3"><i class="fas fa-spinner fa-spin text-muted"></i> Deep Scanning All Folders (Bypassing Limits)...</div>';

    try {
        let allLiveClasses = [];
        let scannedFolders = new Set(); 

        // 🚨 SEQUENTIAL SCANNER: Ek-ek karke scan karega taaki API block na ho. Depth 5 tak jayega.
        async function scanFolderTree(fId, depth) {
            ...
            try {
                ...
                items.forEach(item => {
                    ...
                    let isLive = false;
                    let lStatus = parseInt(d.live_status);
                    
                    // Ultra-Aggressive Live Detection
                    if (type === 'live') isLive = true;
                    if (d.is_live == 1 || d.is_live == "1") isLive = true;
                    if (lStatus === 0 || lStatus === 1) isLive = true;
                    
                    // Reject Recorded Classes (status 2)
                    if (lStatus === 2) isLive = false;
                ...
                // Sequential loop to prevent 429 Too Many Requests
                for (let subId of subFolders) {
                    await scanFolderTree(subId, depth + 1);
                }
            } catch (e) {
                console.warn(`Skipped folder ${fId} due to API block.`);
            }
        }
...
window.fetchFolderContent = async function(folderId) {
    ...
    items.forEach((item, idx) => {
        ...
        let lStatus = parseInt(d.live_status);
        if (type === 'live' || d.is_live == 1 || d.is_live == "1" || lStatus === 0 || lStatus === 1) {
            isLiveClass = true;
            isVideo = true; 
            ...
            if (lStatus === 1 || (now >= liveFrom && lStatus !== 2 && liveFrom > 0)) {
...
window.executeMediaAction = async function(contentId, parentId, title, forceLiveRoute) {
    ...
    try {
        // DUAL METHOD ATTACK (GET then POST)
