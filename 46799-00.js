...
// Deep Folder Scanner + Strict Video_Type:3 Live Routing
// ==========================================================
...
// ==========================================
// 3. LIVE FEED (STRICT TYPE-3 SCANNER)
// ==========================================
async function fetchLiveFeed(courseId) {
    ...
    liveContainer.innerHTML = '<div class="text-center py-3"><i class="fas fa-spinner fa-spin text-muted"></i> Intercepting Live Feeds...</div>';
    ...
    try {
        ...
        async function scanFolderTree(fId, depth) {
            ...
            try {
                ...
                items.forEach(item => {
                    ...
                    let lStatus = parseInt(d.live_status || -1);

                    // 🚨 MASTER KEY: Agar Video Type 3 hai ya Live Status 0 (Scheduled) / 1 (Live) hai
        ...
        allLiveClasses.forEach(item => {
            ...
            // 🕒 CHECK TIME: Agar liveFrom ka time aa chuka hai (diff <= 0) toh LIVE, warna SCHEDULED
            if (diff <= 0) {
...
// ==========================================
// 4. VOD CONTENT FOLDERS (STRICT ROUTING)
// ==========================================
window.fetchFolderContent = async function(folderId) {
    ...
    items.forEach((item, idx) => {
        ...
        // 🚨 STRICT CHECK: Only Video Type 3 or Status 0/1 routes to Live.php
        if (vType === 3 || lStatus === 0 || lStatus === 1) {
            let liveFrom = parseInt(d.live_from || 0) * 1000;
            let diff = liveFrom - now;
        ...
        // Agar Recorded VOD (Type 4) hai, toh routeToLivePHP = false rakho
        else if (isVideo) {
...
window.executeMediaAction = async function(contentId, parentId, title, forceLiveRoute) {
    ...
    try {
        ...
        // 🚨 STRICT CHECK: Open in Live.php ONLY IF forceLiveRoute is true OR video_type is 3
        let videoType = parseInt(mediaData.video_type || 0);
        let isLiveClass = forceLiveRoute || (videoType === 3);
