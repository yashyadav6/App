...
// Custom Ranges + Deep Folder Live Scanner + Parent ID Media Extractor
// ==========================================================
...
// ==========================================
// 3. LIVE FEED (RECURSIVE DEEP SCANNER)
// ==========================================
async function fetchLiveFeed(courseId) {
    ...
    liveContainer.innerHTML = '<div class="text-center py-3"><i class="fas fa-spinner fa-spin text-muted"></i> Deep Scanning All Folders...</div>';

    try {
        let allLiveClasses = [];
        let scannedFolders = new Set(); 

        // 🚨 RECURSIVE SCANNER: Scans up to 3 levels deep
        async function scanFolderTree(fId, depth) {
            if(depth > 3 || scannedFolders.has(fId)) return;
            ...
            items.forEach(item => {
                ...
                if (d.is_live == 1 || d.live_status == 0 || d.live_status == 1 || item.type === 'live') {
                    if (d.live_status !== 2) { 
                        item.parent_folder_id = fId; // Inject Parent ID
        ...
        if (allLiveClasses.length === 0) { 
            liveContainer.innerHTML = `
                <div class="text-center py-4" style="background: var(--bg-color); border-radius: 12px; border: 1px dashed var(--border-color);">
                    ...
                    <p style="color: var(--text-muted); font-weight: 600; margin: 0;">No upcoming live classes found.</p>
                </div>`;
            return; 
        }
...
window.executeMediaAction = async function(contentId, parentId, title, forceLiveRoute) {
    ...
    try {
        // 🚨 THE MASTER FIX: GET aur POST dono Methods ka Dual-Attack Parent ID ke sath!
        let data = await engineFetch(`${API.MEDIA}?content_id=${contentId}&course_id=${currentCourseId}&parent_id=${parentId}`, 'GET');
        
        // Agar GET fail ho jaye, toh POST se try karo! (Bulletproof Logic)
        if (!data || !data.data) {
        ...
        if (isLiveClass) {
            ...
            // Matrix mein chatNode, startTime, aur file URL send ho raha hai
