...
// Strict API Matching + Sniper Fetch + Smooth Red UI
// ==========================================================
...
// 🚨 EXACT VIP HEADERS FROM YOUR OFFICIAL LOGS
const NT_HEADERS = {
...
// ==========================================
// 🛡️ SNIPER FETCHER (One exact hit, no loops)
// ==========================================
async function engineFetch(targetUrl, methodToTry, payload = null) {
    try {
        ...
        // Only attach payload if it's a POST request (Fixes the Firewall Block)
        if (payload && methodToTry === 'POST') {
...
window.executeMediaAction = async function(contentId, title, forceLiveRoute, btnElement) {
    ...
    try {
        ...
        // Pass strictly as GET to our sniper fetcher with null payload
