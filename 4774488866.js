...
// Deep Folder Scanner + Pure Time-Based Live Detection
// ==========================================================
...
// ==========================================
// 3. LIVE FEED (GOD-EYE: PURE TIME-BASED SCANNER)
// ==========================================
async function fetchLiveFeed(courseId) {
    ...
    liveContainer.innerHTML = '<div class="text-center py-3"><i class="fas fa-spinner fa-spin text-muted"></i> Scanning all folders for Time Tags...</div>';

    try {
        let allLiveClasses = [];
        let scannedFolders = new Set(); 
        const now = Date.now();
        // Cutoff: Pichle 24 ghante se purani recorded classes ko top banner mein nahi dikhayenge
        ...
        async function scanFolderTree(fId, depth) {
            ...
            try {
                const data = await engineFetch(API.CONTENT, 'POST', { 
                    course_id: String(courseId), folder_id: String(fId), limit: "5000", page: "1", parent_course_id: "0" 
                });
                ...
                items.forEach(item => {
                    ...
                    let liveFrom = parseInt(d.live_from || 0);
                    let hasChat = d.mqtt_live_cred && Object.keys(d.mqtt_live_cred).length > 0;
                    
                    // 🚨 PURE LOGIC: Agar time hai ya chat hai ya type live hai -> YE LIVE CLASS HAI!
                    if (liveFrom > 0 || hasChat || type === 'live' || d.is_live == 1) {
                        
                        // Check if it's recent or in the future
                        if ((liveFrom * 1000) >= cutoffTime || liveFrom === 0) {
                ...
                for (let subId of subFolders) {
                    await scanFolderTree(subId, depth + 1);
                }
            } catch (e) {
                console.warn(`Folder skipped: ${fId}`);
            }
        }

        await scanFolderTree("0", 1);

        // Sort classes to show upcoming ones first
        allLiveClasses.sort((a, b) => {
            let timeA = parseInt(a.data?.live_from || 0);
            let timeB = parseInt(b.data?.live_from || 0);
            return timeA - timeB;
        });
        ...
        if (allLiveClasses.length === 0) { 
            liveContainer.innerHTML = `
                <div class="text-center py-4" style="background: var(--bg-color); border-radius: 12px; border: 1px dashed var(--border-color);">
                    ...
                    <p style="color: var(--text-muted); font-weight: 600; margin: 0;">No upcoming live classes detected.</p>
                </div>`;
            return; 
        }
        ...
        allLiveClasses.forEach(item => {
            ...
            // Agar time future mein hai -> SCHEDULED
            if (diff > 0) {
                let h = Math.floor(diff / 3600000);
                let m = Math.floor((diff % 3600000) / 60000);
                tagHtml = `<span class="tag-scheduled" style="background:#f59e0b; padding:2px 6px; border-radius:4px; font-size:10px; color:#000; font-weight:bold; margin-left:5px;">🕒 STARTS IN ${h > 0 ? h + 'h ' : ''}${m}m</span>`;
                btnHtml = `<button class="btn-outline" style="color:#f59e0b; border-color:#f59e0b;" onclick="executeMediaAction('${id}', '${parentId}', '${safeTitle}', true)"><i class="fas fa-clock"></i> Waiting Room</button>`;
            } 
            // Agar class start huye 4 ghante se kam hue hain -> LIVE NOW
            else if (diff <= 0 && diff > -(4 * 3600000)) {
            ...
            // Agar bohot purani hai -> RECORDED
            else {
                ...
                btnHtml = `<button class="play-btn" style="background:var(--primary); color:#fff;" onclick="executeMediaAction('${id}', '${parentId}', '${safeTitle}', false)"><i class="fas fa-play"></i> Watch</button>`;
            }

            let dateString = liveFrom > 0 ? new Date(liveFrom).toLocaleString() : "Time not set";
            ...
            <div class="folder-item animate-slide-up">
                <div class="thumb-container">
                    ...
                    <div>
                        ...
                        <small style="color: var(--text-muted);"><i class="far fa-calendar"></i> ${dateString}</small>
                    </div>
                </div>
                ${btnHtml}
            </div>`;
        ...
        liveContainer.innerHTML = '<p class="text-danger text-center">Scan failed. Server blocked the request.</p>';
    }
}
    ...
    items.forEach((item, idx) => {
        ...
        if (liveFrom > 0 || hasChat || type === 'live' || d.is_live == 1) {
            ...
            if (diff > 0) {
                let h = Math.floor(diff / 3600000);
                let m = Math.floor((diff % 3600000) / 60000);
                liveTag = ` <span style="background:#f59e0b; padding:2px 6px; border-radius:4px; font-size:10px; color:#000; font-weight:bold; margin-left:5px;">🕒 STARTS IN ${h > 0 ? h + 'h ' : ''}${m}m</span>`;
...
// ==========================================
// 5. MEDIA ROUTING (WITH PARENT_ID INJECTION)
// ==========================================
window.executeMediaAction = async function(contentId, parentId, title, forceLiveRoute) {
    ...
    try {
        ...
        let hasChat = mediaData.mqtt_live_cred && Object.keys(mediaData.mqtt_live_cred).length > 0;
        let isLiveClass = forceLiveRoute || hasChat || liveFrom > 0;
        
        let chatNode = hasChat ? (mediaData.mqtt_live_cred.public_chat_node || "") : "";
