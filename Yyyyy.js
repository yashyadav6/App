...
// Fast Folder Scan + Strict Status Logic + Exact API Hit
// ==========================================================
...
window.renderNextBatchChunk = async function() {
    ...
    for (let i of chunk) {
        ...
        try {
            // Strictly passing strings to prevent API block
...
// ==========================================
// 3. LIVE FEED (Fast Folder Based Detection)
// ==========================================
async function fetchLiveFeed(courseId) {
    ...
    try {
        ...
        async function scanFolderTree(fId, depth) {
            ...
            try {
                ...
                for (let item of items) {
                    ...
                    // 🚨 Tumhara Logic: Agar Video Type 3 hai, toh wo Live/Scheduled List mein aayega!
                    if (vType === 3) {
        ...
        allLiveClasses.forEach(item => {
            ...
            // 🚨 Root Status Extraction (0 = Scheduled, 1+ = Live)
...
window.fetchFolderContent = async function(folderId) {
    ...
    // Using strictly strings to prevent server 400 errors
    ...
    items.forEach((item, idx) => {
        ...
        // FOLDER RULES (SAME AS LIVE FEED)
        if (vType === 3) {
            if (lStatus === 0) {
                let timeStr = "Soon";
                if (liveFrom > 0 && diff > 0) {
...
// ==========================================
// 5. THE FINAL CLICK - SAFE API FETCHER
// ==========================================
window.executeMediaAction = async function(contentId, parentId, title, forceLiveRoute) {
    ...
    try {
        // 🚨 THE PERFECT GET REQUEST (NO PARENT_ID TO PREVENT SERVER BLOCK)
        let data = await engineFetch(`${API.MEDIA}?content_id=${contentId}&course_id=${currentCourseId}`, 'GET');
        
        // Fallback POST only if GET fails
        if (!data || !data.data) {
