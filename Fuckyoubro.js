...
// Bulletproof 4-Way Media Fetcher + Exact Time Logic
// ==========================================================
...
async function fetchLiveFeed(courseId) {
    ...
    try {
        ...
        async function scanFolderTree(fId, depth) {
            ...
            try {
                ...
                for (let item of items) {
                    ...
                    // If it is Video Type 3, it belongs to Live Feed
                    if (vType === 3) {
        ...
        allLiveClasses.forEach(item => {
            ...
            // STRICT LOGIC FOR TYPE-3 CLASSES
            if (lStatus === 0) {
                let timeStr = "Soon";
...
// ==========================================
// 5. THE FINAL CLICK - BULLETPROOF 4-WAY API FETCHER
// ==========================================
window.executeMediaAction = async function(contentId, parentId, title, forceLiveRoute) {
    ...
    try {
        ...
        // 🚨 4-WAY BRUTE FORCE ATTACK TO GUARANTEE MEDIA FETCH
        const attempts = [
            { method: 'POST', payload: { content_id: contentId, course_id: currentCourseId, parent_id: parentId } }, // Attempt 1 (Standard VOD)
            { method: 'POST', payload: { content_id: contentId, course_id: currentCourseId } }, // Attempt 2 (Standard Live)
            { method: 'GET', url: `${API.MEDIA}?content_id=${contentId}&course_id=${currentCourseId}&parent_id=${parentId}` }, // Attempt 3 (Fallback GET)
            { method: 'GET', url: `${API.MEDIA}?content_id=${contentId}&course_id=${currentCourseId}` } // Attempt 4 (Exact match to your example)
        ];

        for (let att of attempts) {
            let res = att.method === 'POST' 
                ? await engineFetch(API.MEDIA, 'POST', att.payload)
                : await engineFetch(att.url, 'GET');
        ...
        if (!mediaData) {
            alert("Media Payload Blocked. Network Error. Please try again."); return;
        }
