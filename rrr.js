...
// EXACT API MATCHING + PREMIUM RED UI
// ==========================================================
...
// 🚨 EXACT HEADERS FROM YOUR OFFICIAL LOGS
const NT_HEADERS = {
...
async function engineFetch(targetUrl, methodToTry, payload = null) {
    try {
        // Injecting essential headers directly to real HTTP to force blind proxies to forward them
    ...
    try {
        ...
        async function scanFolderTree(fId, depth) {
            ...
            try {
                ...
                // 🚨 EXACT PAYLOAD MATCHING YOUR LOG
                const payload = {
...
window.fetchFolderContent = async function(folderId) {
    ...
    items.forEach((item, idx) => {
        ...
        if (isFolder) {
            ...
            actionHtml = `<button style="${RED_BTN_STYLE}" onmouseover="${RED_BTN_HOVER}" onmouseout="${RED_BTN_OUT}" onclick="executeMediaAction('${contentId}', '${encodedTitle}', ${routeToLivePHP}, this)"><i class="fas ${btnIcon}"></i> ${btnText}</button>`;
        }
...
// ==========================================
// 5. THE EXACT MEDIA FETCHER (STRICTLY URL PARAMS)
// ==========================================
window.executeMediaAction = async function(contentId, title, forceLiveRoute, btnElement) {
    ...
    try {
        // 🚨 EXACT MEDIA FETCH ACCORDING TO YOUR LOG
        // Method: GET
        // URL Params: ?content_id=XYZ&course_id=ABC
        // NO JSON payload. NO parent_id.
        const exactApiUrl = `${API.MEDIA}?content_id=${contentId}&course_id=${currentCourseId}`;
        
        // Pass strictly as GET to our fetcher with null payload
