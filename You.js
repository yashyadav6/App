...
// Omni-Fetcher: Auto-Switches POST/GET with Strict Headers
// ==========================================================

const SP_PROXY = 'nt_proxy.php'; // 🚨 USING YOUR LOCAL PHP PROXY
...
// 🚨 STRICT HEADERS (From your payload)
const NT_HEADERS = {
...
// ==========================================
// 🛡️ OMNI-FETCHER (THE SHAPE-SHIFTER)
// ==========================================
async function engineFetch(targetUrl, preferredMethod, payload = null) {
        ...
        try {
            const response = await fetch(SP_PROXY, {
                ...
                body: JSON.stringify({ 
                    ...
                    method: methodToTry, 
                    payload: payload,
                    headers: NT_HEADERS // Passing strict auth
                })
            });
    ...
    // Attempt 1: The Preferred Method (Usually POST now)
    let res = await hitProxy(preferredMethod);
    if (res && res.success) return res;

    // Attempt 2: If API changed, Shape-Shift to the opposite method!
    let fallbackMethod = preferredMethod === 'POST' ? 'GET' : 'POST';
    console.warn(`[SHAPE-SHIFTER] ${preferredMethod} blocked for ${targetUrl}. Auto-switching to ${fallbackMethod}...`);
    
    res = await hitProxy(fallbackMethod);
    return res;
}
...
window.renderNextBatchChunk = async function() {
    ...
    for (let i of chunk) {
        ...
        try {
            // Using POST as requested by the latest API rules
...
// ==========================================
// 5. THE FINAL CLICK - MEDIA FETCHER
// ==========================================
window.executeMediaAction = async function(contentId, parentId, title, forceLiveRoute) {
    ...
    try {
        // ALWAYS use POST (It auto switches to GET if blocked, thanks to Omni-Fetcher)
        let data = await engineFetch(API.MEDIA, 'POST', { content_id: contentId, course_id: currentCourseId, parent_id: parentId });
