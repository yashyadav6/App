// ==========================================================
// MISSION JEET PREMIUM ENGINE (ORANGE EDITION)
// Strict Vercel Proxy + Mission Jeet Auth Headers
// ==========================================================
...
// 🚨 EXACT HEADERS FOR MISSION JEET FROM YOUR LOG
const MJ_HEADERS = {
    'accept': 'application/json, text/plain, */*',
    'app_id': '1772100600',
    'authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjozMjQ1MDMzLCJhcHBfaWQiOiIxNzcyMTAwNjAwIiwiZGV2aWNlX2lkIjoiZWJhNmI5NzYtOGI5OC00MmVjLWE0MDktMTgwNDc0NDM1YzhiIiwicGxhdGZvcm0iOiIzIiwidXNlcl90eXBlIjoxLCJpYXQiOjE3Nzc3OTQ5MzUsImV4cCI6MTc4MDM4NjkzNX0.IjTLqUdp4zG6lafmYLKFdS-ON-0nAoQi5-AL17ZRn4k',
...
// Premium Amber Button Styles for MJ Theme
const AMBER_BTN_STYLE = `background: linear-gradient(135deg, #f59e0b, #d97706); color: #000; border: none; border-radius: 30px; padding: 10px 20px; font-weight: 800; cursor: pointer; transition: all 0.3s ease; box-shadow: 0 4px 15px rgba(245, 158, 11, 0.4); width: 100%; display: flex; align-items: center; justify-content: center; gap: 8px;`;
const AMBER_BTN_HOVER = `this.style.transform='translateY(-2px)'; this.style.boxShadow='0 6px 20px rgba(245, 158, 11, 0.6)';`;
const AMBER_BTN_OUT = `this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 15px rgba(245, 158, 11, 0.4)';`;

let currentCourseId = null;
let folderHistory = [];
window.masterBatches = [];

// 🚨 ONLY MISSION JEET BATCH IDs
let MASTER_ID_LIST = [153, 152, 151, 150];
...
async function engineFetch(targetUrl, methodToTry, payload = null) {
    try {
        // Real HTTP Headers force Vercel to pass them natively
        const realHttpHeaders = {
            'Content-Type': 'application/json',
            'app_id': MJ_HEADERS.app_id,
            'authorization': MJ_HEADERS.authorization,
            'user_id': MJ_HEADERS.user_id,
            'platform': MJ_HEADERS.platform,
            'version': MJ_HEADERS.version
        };
...
window.renderNextBatchChunk = async function() {
    ...
    if (loadBtn) { loadBtn.style.display = 'block'; loadBtn.innerHTML = "<i class='fas fa-circle-notch fa-spin me-2'></i> Syncing Jeet Matrix..."; }
...
function appendSingleBatch(batch) {
    ...
    container.innerHTML += `
        <div class="course-card">
            ...
            <div style="margin-bottom:12px; font-weight:bold;">₹${batch.price} <span style="font-size:14px; color:var(--text-muted); text-decoration:line-through; font-weight:normal;">₹${batch.mrp}</span></div>
            <button style="${AMBER_BTN_STYLE}" onmouseover="${AMBER_BTN_HOVER}" onmouseout="${AMBER_BTN_OUT}" onclick="openCourse(${batch.id})"><i class="fas fa-rocket"></i> Explore Mission</button>
        </div>`;
}
...
async function fetchLiveFeed(courseId) {
    ...
    liveContainer.innerHTML = '<div class="text-center py-4"><i class="fas fa-circle-notch fa-spin fa-2x text-warning mb-2"></i><br><span class="text-muted">Scanning Live Matrix...</span></div>';
    ...
    try {
        ...
        async function scanFolderTree(fId, depth) {
            ...
            try {
                ...
                // Exact Post Payload as per your logs
                const payload = {
        ...
        allLiveClasses.forEach(item => {
            ...
            if (lStatus === 0) {
                ...
                btnHtml = `<button style="${AMBER_BTN_STYLE} opacity:0.5;" disabled><i class="fas fa-lock"></i> Starts in ${timeStr}</button>`;
            } 
            else {
                ...
                btnHtml = `<button style="${AMBER_BTN_STYLE}" onmouseover="${AMBER_BTN_HOVER}" onmouseout="${AMBER_BTN_OUT}" onclick="executeMediaAction('${id}', '${safeTitle}', true, this)"><i class="fas fa-satellite-dish"></i> Join Live</button>`;
            }
            ...
            <div class="folder-item animate-slide-up">
                <div class="thumb-container">
                    ...
                    <div>
                        ...
                        <small style="color: var(--primary-neon); font-size: 12px;"><i class="far fa-clock"></i> ${dateString}</small>
                    </div>
                </div>
...
window.fetchFolderContent = async function(folderId) {
    ...
    listDiv.innerHTML = "<div class='text-center py-5'><i class='fas fa-circle-notch fa-spin fa-2x text-warning'></i><p class='mt-2 text-muted'>Syncing Vault...</p></div>";
    
    const navDiv = document.getElementById('folder-nav');
    if (navDiv) {
        navDiv.innerHTML = folderHistory.length > 1 
            ? `<button class="btn btn-link text-decoration-none" style="padding:0; color:var(--primary-neon);" onclick="folderHistory.pop(); fetchFolderContent(folderHistory[folderHistory.length-1].id);"><i class="fas fa-level-up-alt"></i> Go Up</button> <h5 style="color: #fff; margin-top:10px;"><i class="fas fa-folder-open text-warning me-2"></i> ${folderHistory[folderHistory.length-1].name}</h5>` 
            : "";
    }

    const payload = {
        course_id: String(currentCourseId),
    ...
    items.forEach((item, idx) => {
        ...
        if (vType === 3) {
            ...
            btnText = 'Watch Recording'; btnIcon = "fa-play-circle";
        ...
        let dateHtml = dateString ? `<br><small style="color: var(--primary-neon); font-size:11px;"><i class="far fa-clock"></i> ${dateString}</small>` : "";

        let actionHtml = '';
        if (isFolder) {
            actionHtml = `<button style="${AMBER_BTN_STYLE}" onmouseover="${AMBER_BTN_HOVER}" onmouseout="${AMBER_BTN_OUT}" onclick="folderHistory.push({id:'${contentId}', name:'${safeTitle}'}); fetchFolderContent('${contentId}');"><i class="fas fa-folder-open"></i> Open Folder</button>`;
        } else if (isBtnDisabled) {
            actionHtml = `<button style="${AMBER_BTN_STYLE} opacity: 0.5;" disabled><i class="fas ${btnIcon}"></i> ${btnText}</button>`;
        } else {
            actionHtml = `<button style="${AMBER_BTN_STYLE}" onmouseover="${AMBER_BTN_HOVER}" onmouseout="${AMBER_BTN_OUT}" onclick="executeMediaAction('${contentId}', '${encodedTitle}', ${routeToLivePHP}, this)"><i class="fas ${btnIcon}"></i> ${btnText}</button>`;
        }
...
window.executeMediaAction = async function(contentId, title, forceLiveRoute, btnElement) {
    ...
    try {
        // 🚨 EXACT MEDIA FETCH ACCORDING TO YOUR LOG (Strict GET)
        ...
        if (forceLiveRoute || videoType === 3) {
            ...
            // Reusing your existing live.php player
        ...
        if (mediaUrl) {
            if (mediaUrl.toLowerCase().endsWith('.pdf') || fileType === 3 || fileType === 1) {
                ...
                // Reusing your existing player.php
